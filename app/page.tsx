    ...
    <div className="flex flex-col items-center justify-center w-screen min-h-screen bg-gradient-to-tl from-black via-zinc-600/20 to-black">
        ...
        <ul className="flex items-center justify-center gap-8 md:gap-4">
          {navigation.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="text-lg duration-500 text-zinc-500 hover:text-zinc-300 md:text-sm"
            >
              {item.name}
            </Link>
          ))}
      ...
      <div className="my-16 text-center animate-fade-in">
        <h2 className="text-sm text-zinc-500 ">
          Software Engineer building AI-powered automation at{" "}
          <Link
            target="_blank"
            href="https://e-tailize.com"
